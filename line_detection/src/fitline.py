...
class line_detection:
    ...
    node_name = "fitline_lanedetection"
    ...
    def image_callback(self, image):
        ...
        # extracts nonzero pixels and formats them as separate points (each point
        # is a vector of two elements, x and y)
        points =  np.float32(np.transpose(roi.nonzero()))
        
        # code from http://stackoverflow.com/a/14192660/341505
        # then apply fitline() function
        [vx,vy,x,y] = cv2.fitLine(points,cv2.cv.CV_DIST_L2,0,0.001,0.001)
        ...
        cv2.line(roi,(roi.shape[1]-1,righty),(0,lefty),255,2)
